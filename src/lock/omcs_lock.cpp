...
OMCSLock::OptGuard::TryLockS(  //
    ...
    -> SGuard
{
  if (has_lock_) return SGuard{std::exchange(dest_, nullptr), qid_};
  ...
  return ((ver_ ^ expected) & mask) ? SGuard{} : SGuard{std::exchange(dest_, nullptr), qid_};
}
